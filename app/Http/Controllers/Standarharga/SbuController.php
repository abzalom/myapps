...
class SbuController extends Controller
{
    public function sbu()
    {
        $kategoris = K2SshKategori::whereIn('kategori_kode', [4])->where('tahun', $this->tahun->tahun)->get('kode_unik_subrincian');
        $kodes = [];
        ...
        $ssh = D6SubrincianLo::with([
            'komponen' => fn ($q) => $q->where(['tahun' => $this->tahun->tahun, 'kategori_kode' => 4]),
            'komponen.zonasi',
            'komponen.typeproduk',
        ])
            ->whereIn('kode_unik_subrincian', $kodes['subrincian'])
            ->where('kode_kategori_ssh', 4)
            ->get();
        return view('standarharga.sbu.sbu', [
            'title' => 'Standar Harga | SBU',
            'desc' => 'Standar Belanja Umum (SBU)',
            'ssh' => $ssh,
            'zonasis' => EZonasi::all(),
        ]);
    }
    ...
    public function sbustore(Request $request)
    {
        ...
        $kategori = D6SubrincianLo::where('kode_unik_subrincian', $request->get('kategori'))->first();
    ...
    public function sbuupload(Request $request)
    {
    ...
    public function sbuupdate(Request $request)
    {
        if ($request->get('kategori') == 0) {
            return redirect()->back()->with('pesan', '<div class="alert alert-warning">Ketegori barang belum dipilih!</div>');
        }
    ...
    public function sbudelete(Request $request)
    {
        $komponen = K3SshKomponen::find($request->get('idkomponen'));
        $komponen->delete();
