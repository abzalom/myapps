...
class HspkController extends Controller
{
    ...
    {
        $kategoris = K2SshKategori::whereIn('kategori_kode', [2])->where('tahun', $this->tahun->tahun)->get('kode_unik_subrincian');
        $kodes = [];
        ...
        $ssh = D6SubrincianLo::with([
            ...
            ->where('kode_kategori_ssh', 2)
            ->get();
        return view('standarharga.hspk.hspk', [
            'title' => 'Standar Harga | HSPK',
            'desc' => 'Harga Satuan Pokok Kegiatan (HSPK)',
            'ssh' => $ssh,
            'zonasis' => EZonasi::all(),
        ]);
    }

    public function hspkstore(Request $request)
    {
    ...
    public function hspkupdate(Request $request)
    {
        if ($request->get('kategori') == 0) {
            return redirect()->back()->with('pesan', '<div class="alert alert-warning">Ketegori barang belum dipilih!</div>');
        }
    ...
    public function hspkdelete(Request $request)
    {
        $komponen = K3SshKomponen::find($request->get('idkomponen'));
        $komponen->delete();
